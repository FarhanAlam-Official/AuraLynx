...
def generate_instrumental(request):
    ...
    try:
        ...
        audio_url = f'http://localhost:8000/temp-audio/{filename}'
...
def mix_audio(request):
    ...
    try:
        ...
        # Mix audio (note: instrumental_url and vocals_url are HTTP URLs, need to convert back to paths)
        # Extract filenames from URLs and convert to local paths
        instrumental_filename = instrumental_url.split('/')[-1]
        vocals_filename = vocals_url.split('/')[-1]
